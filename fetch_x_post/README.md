...
```bash
fetch_x_post_image$ sam local invoke FetchXPost --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
fetch_x_post_image$ sam local start-api
fetch_x_post_image$ curl http://localhost:3000/
```
...
```bash
fetch_x_post_image$ sam logs -n FetchXPost --stack-name fetch_x_post_image --tail
```
...
```bash
fetch_x_post_image$ cd hello-world
...
```bash
sam delete --stack-name fetch_x_post_image
```
